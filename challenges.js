...
/*
  Pretty simple. We expect an array of numbers to be passed in, so we can just forEach through, adding to a variable outside the scope of the for.
*/
...
/*
  This challenge is very similar to 03, but the keyword here is ARGUMENTS. We're told to expect all the params to be numbers, which makes things easier. The arguments object is array-LIKE, so not a true array. To coerce(?) it into an actuall array, we can assign a var to Array.from(arguments). Then just set up a conditional to catch when there are no arguments passed in.
*/
...
function computeRemainder(dividend, divisor) {
  ...
  if (divisor === 0) {
    ...
    console.log(`if you used modulo, you get: ${dividend % divisor}`)
    let mult = 1;
    let rem = [];
    let origDiv = divisor;
    let cumuPrd = 1;
    while (dividend > (cumuPrd)) {
      cumuPrd = mult * origDiv;
      mult++;
      let diff = dividend - cumuPrd;
      if (diff === 0) return 0;
      rem.push(dividend - cumuPrd);
      console.log(`rem now contains: ${rem}`);
    }
    return rem[rem.length - 2];
  }
}
...
function range(int1, int2) {
  if (int1 > int2) {
    let msg = "First argument must be less than second";
    return msg;
  }
  let arr = [];
  while (int1 !== int2) {
    console.log(`value of upper limit is: ${Math.abs(int2) - 1}`)
    arr.push(int1);
    int1++;
    console.log(`arr holds: ${arr}. value of int1 is now: ${int1}`)
  }
  return arr;
}
...
function charCount(string) {
  const strArr = string.split(""); //.split() returns an Array
  const charObj = {};
  let uniqChar = [];
  // let's go with .split() since it keeps track of space as well as upper/lower cases
  // console.log(`initial value of charObj: ${charObj}`);
  console.log(charObj);
  console.log(`strArr is: ${strArr}`);

  strArr.forEach(function (char, idx) {
    // console.log(charObj);
    console.log(`the char at strArr[${idx}] is ${strArr[idx]}`);
    console.log(`charObj is: ${charObj}`);

    // NEED TO FIND OUT WHY CHAR ISN'T ADDING PROPERTIES PROPERLY 

    if (!(uniqChar.includes(char))) {
      uniqChar.push(char);
      console.log(`uniqChar: ${uniqChar}`);
      charObj[char] = 1;
      console.log(`in if: value of charObj.char: ${charObj[char]}, value of char: ${char}`)
      // by default sets the value of a new key to be 1; again this is for
      // when we create a UNIQUE char

    } else {
      // if you're in here, that means the char at [i] IS included in uniqChar
      let charVal = charObj[char];
      let newVal = charVal + 1;
      // don't need a counter; just add 1 here; you won't be in the else block if char is unique
      charObj[char] = newVal;
      console.log(`in else: value of charObj.char: ${charObj[char]}`);
      // again, don't need to worry about resetting counter, because it'll be set to 0 at
      // the beginning of each iteration


      // apparently, YOU'RE AN IDIOT. alternatively, you forgot about [] notation. it's more powerful
      // that dot, but less readable. https://medium.com/@prufrock123/js-dot-notation-vs-bracket-notation-797c4e34f01d
    }
  });
  return charObj;
}
...
function formatWithPadding(num, padChar, padLength) {
  ...
  let padArr = [];
  padArr = paddedStr.split();
  console.log(`padArr: ${padArr}`);
  // console.log(`paddedStr val: ${paddedStr} and is typeof: ${typeof paddedStr}`);
  while ((padLength - paddedStr.length) > 0) {
    if (num < padLength) {
      return paddedStr;
    } else {
      padArr.unshift(padChar);
    }
    padLength--;
  };
  return padArr.join('');
...
function hammingDistance(str1, str2) {
  ...
  let diffArr = [];
  function findDiff(charAt, idx) {
    if (charAt !== str2.split("")[idx]) return charAt;
  };
  return str1.split("").filter(findDiff).length;
}
...
function mumble(strArg) {
  let mumArr = [];
  strArg.split("").forEach(function (charAt, idx) {
    let counter = idx + 1;
    let tempArr = [];
    while (counter > 0) {
      tempArr.push(charAt);
...
function fromPairs() {
  let argArr = Array.prototype.slice.call(arguments)[0]; //nice line. found in arguments MDN docu
  let obj = {};
...
function mergeObjects(obj1, obj2, ...moreObjs) {
  let argArr = Array.prototype.slice.call(arguments); //nice line. found in arguments MDN docu
  let target = obj1;
  argArr.forEach(function (objArg) {
    target = Object.assign(target, objArg);
    console.log(target);
  })
  return target;
}
...
function findHighestPriced(arrOfObj) {
  let mostExp = arrOfObj.sort(function (a, b) {
     ...
     * 
     * However, with the if statement above, we change it so that if a.price===b.price,
     * then we sorta a BEFORE b (this is important because we reverse the order with
     * a method chain and because we will shift the first element of the newly sorted
     * array)
     */
  }).reverse();
  return mostExp.shift();
}
...
function mapArray(argArr, cb) {
  newArr = [];
  argArr.forEach((elem, idx) => {
...
function reduceArray(argArr, cb, acc) {
  let newAcc;
  argArr.forEach((elem, idx) => {
    if (idx === 0) {
      newAcc = cb(acc, elem, idx);
    } else {
      newAcc = cb(newAcc, elem, idx);
    }
  });
  return newAcc;
}
...
function flatten(arr) {
  ...
  function recurFlatten(arr) {
    arr.forEach((elem) => {
      if (typeof elem !== "object") {
        // right now, this will pass the tests because the tests are nice and only pass in arrays
        // which resolve to typeof "object"; but i'm pretty sure that an error will be thrown if 
        // there's ever an object literal in the originally passed-in array
        flattened.push(elem);
...
function primeFactors(num){
  ...
  // we see it for the second time today (jun 5, 2019); they take advantage of 0 being a FALSEY value
  // so if n%number === 0, that means n is a multiple of number; therefore, because we want to KEEP 
  // this value that's a multiple, we just ! (not) the computation and we have an ELOQUENT solution
  // to filtering out non-multiples


// https://stackoverflow.com/questions/54193060/javascript-function-for-finding-multiples-of-a-number
